    ...
    return (
        <>
            <div className="mb-3 container" style={{color:prop.mode==="dark"?"white":"black"}}>
                <h3  >{prop.heading}</h3>
                <textarea className="form-control" style={{backgroundColor:prop.mode==="dark"?"black":"white",color:prop.mode==="dark"?"white":"black"}} value={text} onChange={handleOnChane} id="myBox" rows="5"></textarea>
                ...
                <button className="btn btn-primary my-3 mx-3" onClick={handleSpacesclick}>Remove Extra Spaces</button>

            </div>
            <div className="container" style={{color:prop.mode==="dark"?"white":"black"}}>
                <h3>Text Summary</h3>
                <p> {text.split(" ").length} word , {text.length} character  </p>
                <p>{text.split(" ").length*0.008} minitue to read</p>
                <h4>Preview</h4>
                <p>{text}</p>
            </div>
